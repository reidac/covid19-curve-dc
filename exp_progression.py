...
popt,pcov = curve_fit(model, x, y, p0=[1,1],sigma=None, method="lm",
                      jac=jacobian)
...
upper = model(bhat, popt[0]+perr[0], popt[1]+perr[1])
...
for panel in range(start_n,len(x)):
    ...
    popt,pcov = curve_fit(model, x[:panel], y[:panel], p0=[1,1],
    ...
    if "FIG_PATH" in os.environ:
        ...
        plt.savefig("{0}/us_dc_panel_{1:02d}.png".format(fig_path,panel),
